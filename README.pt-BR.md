...
O projeto é um reactor Maven multi-módulo com 4 módulos e dependência acíclica entre eles: `core`
e `translate` não dependem de nada no projeto; `frama-c` depende dos dois; `cli` depende dos três.

```mermaid
flowchart TB
    AB["Atelier B<br/>(external tool via plugin.etool)"] -->|"caminho bdp"| CLI
    ...
    subgraph FRAMAC["frama-c"]
        direction TB
        FCR["FramaCRunner"]
        POST["Estágios de pós-processamento<br/>de merged_code.c"]
        FCR --> POST
    end
    ...
    FRAMAC -->|"frama-c -acsl-import / -wp"| FC["Frama-C + prover SMT<br/>(CVC5 / Alt-Ergo / Z3)"]
    FC -->|"código de saída"| AB
```

`translate` é o módulo verdadeiramente standalone (zero dependências de outro módulo do
projeto) — é ele que embarca a tradução BXML → ACSL e a biblioteca `B2ACSLLib` (submódulo Git),
podendo ser consumido isoladamente por outra ferramenta JVM sem trazer Swing/AWT nem a invocação
do Frama-C junto.
